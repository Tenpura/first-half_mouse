...
private:
	...
	static void set_maze_wall( MAZE_WALL* const maze_wall,unsigned char edit_number,unsigned char set_number);//maze_wallに値を代入する用
...
public:
	...
	static signed char get_wall(unsigned char wall_x, unsigned char wall_y, unsigned char muki);
	...
	static signed char check_exist(unsigned char wall_x, unsigned char wall_y, unsigned char muki);

	static void reset_wall();		//壁情報と見たかどうかをすべてリセット

	static void draw_map();//myprintfで迷路を描画
